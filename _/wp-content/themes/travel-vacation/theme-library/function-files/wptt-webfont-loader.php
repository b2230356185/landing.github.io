...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $travel_vacation_url The remote URL.
		 */
		public function __construct( $travel_vacation_url = '' ) {
			$this->remote_url = $travel_vacation_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $travel_vacation_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $travel_vacation_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $travel_vacation_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $travel_vacation_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $travel_vacation_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $travel_vacation_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $travel_vacation_format = 'woff2' ) {
			$this->font_format = $travel_vacation_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $travel_vacation_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $travel_vacation_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $travel_vacation_url, '/' ) ) {
				$travel_vacation_parsed_url = parse_url( $this->remote_url );
				return $travel_vacation_parsed_url['scheme'] . '://' . $travel_vacation_parsed_url['hostname'] . $travel_vacation_url;
			}

			return $travel_vacation_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $travel_vacation_url    The URL of the remote webfont.
	 * @param string $travel_vacation_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $travel_vacation_url, $travel_vacation_format = 'woff2' ) {
		$travel_vacation_font = new WPTT_WebFont_Loader( $travel_vacation_url );
		$travel_vacation_font->set_font_format( $travel_vacation_format );
		return $travel_vacation_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $travel_vacation_url, $travel_vacation_format = 'woff2' ) {
		$travel_vacation_font = new WPTT_WebFont_Loader( $travel_vacation_url );
		$travel_vacation_font->set_font_format( $travel_vacation_format );
		return $travel_vacation_font->get_url();
	}
}